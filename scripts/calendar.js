...
function calendarCaption(calDate) {
  ...
  let calCaption =
    "<caption>" + monthName[thisMonth] + " " + thisYear + "</caption>";
...
function calendarWeekDayRow(calDate) {
  ...
  for (let dayIndex = 0; dayIndex < dayName.length; dayIndex++) {
    rowHTML += "<th id='calendar-weekdays'>" + dayName[dayIndex] + "</th>";
  }
  rowHTML += "</tr>";
...
function calendarDays(calDate) {
  ...
  for (let days = 1; days < thisSun; days++) {
    ...
    htmlCode += "<td class='calendar-dates'>" + days + "</td>";
    if (weekDay === 6) {
      htmlCode += "</tr>";
    }
  }
  ...
  for (let days = thisSun; days <= thisSun + 6; days++) {
    if (days === highLightDay) {
      htmlCode +=
        "<td class='calendar-dates' id='calendar-today'>" +
        days +
        dayDeal[highLightDay - thisSun] +
        "</td>";
    } else {
      htmlCode +=
        "<td class='calendar-dates'>" +
        days +
        dayDeal[days - thisSun] +
        "</td>";
    }
  }
  htmlCode += "</tr>";
